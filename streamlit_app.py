...
if uploaded_file is not None:
    ...
    dependent_var = st.selectbox("Select the dependent variable (Response)", columns)
    treatment_var = st.selectbox("Select the treatment variable", columns)

    if dependent_var and treatment_var:
        # Perform ANOVA
        model, anova_table = perform_anova(data, dependent_var, treatment_var)
        st.write("ANOVA Summary")
        st.write(anova_table)
        
        # Perform LSD Test
        lsd_result = perform_lsd(data, treatment_var, dependent_var)
        st.write("LSD Test Summary")
        st.text(lsd_result)
        
        # Perform DMRT Test
        dmrt_result = perform_dmrt(data, treatment_var, dependent_var)
